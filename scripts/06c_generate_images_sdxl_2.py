...
===========================
Upgraded generation script using SSD-1B (Distilled SDXL)
Optimized for 6GB VRAM GPUs.

WHY SSD-1B IS BETTER FOR YOUR PROJECT:
  - Generates 1024x1024 images (vs 512x512 in SD 1.5)
  - Distilled from SDXL: 1.3 Billion parameters (vs 3.5B), saving massive VRAM.
  ...
  - Uses sequential layer offloading and VAE slicing to prevent OOM errors.
...
# ── SDXL model (Distilled for lower VRAM) ─────────────────────
...
def main():
    ...
    print("   First run downloads ~5GB — this is normal.")
    ...
    # ── AGGRESSIVE MEMORY OPTIMISATION FOR 6GB VRAM ──────────
    if device.type == "cuda":
        # 1. Sequential offload (layer-by-layer)
        pipe.enable_sequential_cpu_offload()
        # 2. Slice attention
        pipe.enable_attention_slicing()
        # 3. Slice and tile VAE to prevent OOM at final decode
    ...
    if not os.path.exists(split_csv):
        ...
        # Ensure it is moved to the correct device/dtype
