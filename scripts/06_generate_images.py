...
INPUT_FILE = r"D:/PBL_6/DATA_prep/output/models/pred_scenes.npy"  
...
def generate():
    ...
    print(f"  predictions from {pred_file}...")
    try:
    ...
    if embeddings.shape[-1] != text_encoder_dim:
        print(
            f"   Creating projection layer: {embeddings.shape[-1]} -> {text_encoder_dim}"
        )
    ...
    print(
        f"   This will take approximately {num_to_generate * 0.5:.1f} minutes on GPU..."
    )
    ...
    for i in range(num_to_generate):
        if i % 10 == 0:
            print(
                f"   - Processing images {i}-{min(i+10, num_to_generate)} of {num_to_generate}..."
            )

        # Get single brain signal: Shape (1024,)
        brain_signal = (
            embeddings[i].unsqueeze(0).to(device).to(dtype)
        )  # (1, 1024) - match dtype!
