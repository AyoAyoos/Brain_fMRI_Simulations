...
def calculate_metrics():
    ...
    if not os.path.exists(recon_folder):
        print(f"❌ CRITICAL ERROR: Recon folder not found at {os.path.abspath(recon_folder)}")
    ...
    for i, row in df.iterrows():
        ...
        if not os.path.exists(real_path):
            if i < 3: print(f"      ⚠️ Missing REAL image: {row['filename']}")
            continue
            
        if not os.path.exists(recon_path):
            if i < 3: print(f"      ⚠️ Missing RECON image: recon_{i:03d}.png")
            continue
