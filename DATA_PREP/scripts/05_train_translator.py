...
def train_and_test():
    ...
    try:
        ...
        score_scene = evaluate_predictions(Y_scene, Y_pred_scene, "Complex Scenes (COCO)")
    ...
    if 'score_obj' in locals() and 'score_scene' in locals():
        ...
        print(f"   Gap: {diff:.4f}")
        if diff > 0:
            print("   ✅ HYPOTHESIS CONFIRMED: The model understands Objects better than Scenes.")
        else:
            print("   ❓ RESULT: Scores are similar (or Scenes are better). Interesting!")
